...
use web_sys::
{
    GpuDevice, GpuBufferDescriptor, GpuShaderModuleDescriptor, GpuProgrammableStage, GpuComputePipelineDescriptor, 
    GpuBufferBinding, GpuBindGroupEntry, GpuBindGroupDescriptor, GpuBindGroupLayoutEntry, GpuBufferBindingLayout,
    GpuBufferBindingType, GpuBindGroupLayoutDescriptor, GpuPipelineLayoutDescriptor,

    // GpuAdapter, 
    // Window, WorkerGlobalScope, WorkerNavigator,
};

use web_sys::gpu_buffer_usage::{STORAGE, COPY_SRC, UNIFORM, MAP_READ, COPY_DST};
    ...
    b_elements_values: Vec<f32>) -> Result<JsValue, JsValue>
{
    ...
    let iteration_number_usage = STORAGE;
    ...
    let mut bind_group_layout_entry_0: GpuBindGroupLayoutEntry = GpuBindGroupLayoutEntry::new(0, COMPUTE);
    let mut bind_group_layout_entry_0_buffer = GpuBufferBindingLayout::new();
    bind_group_layout_entry_0_buffer.type_(GpuBufferBindingType::Storage);
    bind_group_layout_entry_0.buffer(&bind_group_layout_entry_0_buffer);

    let mut bind_group_layout_entry_1: GpuBindGroupLayoutEntry = GpuBindGroupLayoutEntry::new(1, COMPUTE);
    let mut bind_group_layout_entry_1_buffer = GpuBufferBindingLayout::new();
    bind_group_layout_entry_1_buffer.type_(GpuBufferBindingType::Uniform);
    bind_group_layout_entry_1.buffer(&bind_group_layout_entry_1_buffer);

    let mut bind_group_layout_entry_2: GpuBindGroupLayoutEntry = GpuBindGroupLayoutEntry::new(2, COMPUTE);
    ...
    let mut bind_group_layout_entry_3: GpuBindGroupLayoutEntry = GpuBindGroupLayoutEntry::new(3, COMPUTE);
    let mut bind_group_layout_entry_3_buffer = GpuBufferBindingLayout::new();
    bind_group_layout_entry_3_buffer.type_(GpuBufferBindingType::Storage);
    bind_group_layout_entry_3.buffer(&bind_group_layout_entry_3_buffer);

    let bind_group_layout_entries = [
        bind_group_layout_entry_0,
        bind_group_layout_entry_1,
        bind_group_layout_entry_2,
        bind_group_layout_entry_3,
    ].iter().collect::<js_sys::Array>();

    let mut bind_group_layout_descriptor = GpuBindGroupLayoutDescriptor::new(&bind_group_layout_entries);
    bind_group_layout_descriptor.label("NGE Bind Group Layout");
    ...
    pipeline_layout_descriptor.label("NGE Pipeline Layout");
    ...
    let gpu_compute_pipeline_descriptor = GpuComputePipelineDescriptor::new(
    ...
    let gpu_bind_group_entries = [
        gpu_bind_group_entry_a,
        gpu_bind_group_entry_a_shape,
        gpu_bind_group_entry_b,
        gpu_bind_group_entry_iteration_number,
    ].iter().collect::<js_sys::Array>();
    ...
    gpu_compute_pass_encoder.set_bind_group(0u32, Some(&gpu_bind_group));

    for _ in init_iteration..a_rows_number * 2
    {
        gpu_compute_pass_encoder.dispatch_workgroups(a_rows_number);
    }

    gpu_compute_pass_encoder.end();
    
    let read_usage = COPY_DST | MAP_READ;
    ...
    JsFuture::from(gpu_buffer_read.map_async(READ)).await?;
